...
tape('PARSER / if user.account is in segment all then split 100:on', function (assert) {
  ...
  assert.true(evaluator('test@split.io', 31) === undefined, "missing attribute account ends in undefined result");
...
tape('PARSER / if user.attr is between 10 and 20 then split 100:on', function (assert) {
  ...
  assert.true(evaluator('test@split.io', 31) === undefined, 'undefined is not between 10 and 20');
