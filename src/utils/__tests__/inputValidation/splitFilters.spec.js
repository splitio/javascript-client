...
tape('INPUT VALIDATION for splitFilters', t => {
  ...
  t.test('Returns default output with empty values if `splitFilters` is an invalid object or `mode` is not \'standalone\'', assert => {
    ...
    assert.true(loggerMock.warn.getCall(0).calledWithExactly('splitFilters configuration must be a non-empty array of filter objects.'));

    assert.deepEqual(validateSplitFilters(15, STANDALONE_MODE), defaultOutput, 'splitFilters ignored if not a non-empty array');
    assert.true(loggerMock.warn.getCall(1).calledWithExactly('splitFilters configuration must be a non-empty array of filter objects.'));

    assert.deepEqual(validateSplitFilters('string', STANDALONE_MODE), defaultOutput, 'splitFilters ignored if not a non-empty array');
    assert.true(loggerMock.warn.getCall(2).calledWithExactly('splitFilters configuration must be a non-empty array of filter objects.'));

    assert.deepEqual(validateSplitFilters([], STANDALONE_MODE), defaultOutput, 'splitFilters ignored if not a non-empty array');
    assert.true(loggerMock.warn.getCall(3).calledWithExactly('splitFilters configuration must be a non-empty array of filter objects.'));
  ...
  t.test('Returns object with null queryString, if `splitFilters` contains invalid filters or contains filters with no values or invalid values', assert => {
    ...
    assert.true(loggerMock.debug.getCall(0).calledWithExactly("splits filtering criteria is 'null'."));
    loggerMock.debug.resetHistory();
    ...
    assert.true(loggerMock.debug.getCall(0).calledWithExactly("splits filtering criteria is 'null'."));
    assert.true(loggerMock.warn.getCall(0).calledWithExactly("split filter at position '3' is invalid. It must be an object with a valid filter type ('byName' or 'byPrefix') and a list of 'values'."), 'invalid value of `type` property');
    assert.true(loggerMock.warn.getCall(1).calledWithExactly("split filter at position '4' is invalid. It must be an object with a valid filter type ('byName' or 'byPrefix') and a list of 'values'."), 'invalid type of `values` property');
    assert.true(loggerMock.warn.getCall(2).calledWithExactly("split filter at position '5' is invalid. It must be an object with a valid filter type ('byName' or 'byPrefix') and a list of 'values'."), 'invalid type of `type` property');
  ...
  t.test('Returns object with a queryString, if `splitFilters` contains at least a valid `byName` or `byPrefix` filter with at least a valid value', assert => {
    ...
    for (let i = 0; i < splitFilters.length; i++) {
      ...
      if (groupedFilters[i]) { // tests where validateSplitFilters executes normally
        ...
        assert.true(loggerMock.debug.calledWith(`splits filtering criteria is '${queryStrings[i]}'.`));
