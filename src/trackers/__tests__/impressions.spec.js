...
tape('Impressions Tracker / API', assert => {
...
tape('Impressions Tracker / Should be able to queue elements and track them on demand.', assert => {
  const { fakeStorage, fakeSettings } = generateContextMocks();
  const contextMock = new ContextMock(fakeStorage, fakeSettings);
...
tape('Impressions Tracker / Queued impressions should be sent to impression listener when we invoke .track()', assert => {
