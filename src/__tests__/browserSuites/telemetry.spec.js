...
export default async function telemetryBrowserSuite(fetchMock, t) {
  ...
  t.test(async (assert) => {
    fetchMock.getOnce(baseUrls.sdk + '/splitChanges?v=1.0&since=-1', 500);
    fetchMock.getOnce(baseUrls.sdk + '/splitChanges?v=1.0&since=-1', { status: 200, body: splitChangesMock1 });
  ...
  t.test(async (assert) => {
    ...
    fetchMock.getOnce(baseUrls.sdk + '/splitChanges?v=1.0&since=-1&sets=a,c,d',  { status: 200, body: { splits: [], since: 1457552620999, till: 1457552620999 } });
    fetchMock.postOnce(baseUrls.telemetry + '/v1/metrics/config', (url, opts) => {
      const data = JSON.parse(opts.body);
