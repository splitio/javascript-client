...
export default function readyPromiseAssertions(fetchMock, assert) {
  ...
  assert.test(t => {
    ...
    fetchMock.getOnce(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) + 20 });
    fetchMock.getOnce(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) + 20 });
  ...
  assert.test(t => {
    ...
    fetchMock.getOnce(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) + 20 });
    fetchMock.getOnce(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) + 20 });
    fetchMock.getOnce(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: refreshTimeMillis });
    // main client endpoint configured to fetch segments before request timeout
    fetchMock.get(config.urls.sdk + '/mySegments/facundo%40split.io', mySegmentsFacundo, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) - 20 });
    fetchMock.get(config.urls.sdk + '/splitChanges?v=1.0&since=1457552620999', { splits: [], since: 1457552620999, till: 1457552620999 });
  ...
  assert.test(t => {
    ...
    fetchMock.get(config.urls.sdk + '/splitChanges?v=1.0&since=-1', splitChangesMock1, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) + 20 });
    fetchMock.get(config.urls.sdk + '/mySegments/facundo%40split.io', mySegmentsFacundo, { delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) - 20 });
    fetchMock.postOnce(config.urls.events + '/testImpressions/bulk', 200);
    fetchMock.postOnce(config.urls.events + '/testImpressions/count', 200);
