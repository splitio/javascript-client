...
const expectedHashNullFilter = '7601f4e3'; // for SDK key '<fake-token-rfc>', filter query null, and endpoint version '1.0'
const expectedHashWithFilter = 'ebcf61d9'; // for SDK key '<fake-token-rfc>', filter query '&names=p1__split,p2__split', and endpoint version '1.0'
...
export default function (fetchMock, assert) {
  ...
  assert.test(t => { // Testing when we start from scratch
    ...
    fetchMock.get(testUrls.sdk + '/splitChanges?v=1.0&since=-1', { status: 200, body: splitChangesMock1 });
  ...
  assert.test(t => { // Testing when we start with cached data but without lastUpdate item (JS SDK version 10.13.0 and below)
    ...
    fetchMock.get(testUrls.sdk + '/splitChanges?v=1.0&since=25', function () {
  ...
  assert.test(t => { // Testing when we start with cached data but without storage hash (JS SDK <=v10.24.0 and Browser SDK <=v0.12.0), and a valid split filter config
    ...
    fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=-1&names=p1__split,p2__split', { status: 200, body: { splits: [splitDeclarations.p1__split, splitDeclarations.p2__split], since: -1, till: 1457552620999 } }, { delay: 10 }); // short delay to let emit SDK_READY_FROM_CACHE
    // fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=1457552620999&names=p1__split', { status: 200, body: { splits: [], since: 1457552620999, till: 1457552620999 } });
  ...
  assert.test(t => { // Testing when we start with cached data with split filter, and the same split filter is provided in the config
    ...
    fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=25&names=p2__split&prefixes=p1', { status: 200, body: { splits: [splitDeclarations.p1__split, splitDeclarations.p2__split], since: 25, till: 1457552620999 } }, { delay: 10 }); // short delay to let emit SDK_READY_FROM_CACHE
    fetchMock.getOnce(testUrls.sdk + '/mySegments/nicolas%40split.io', { status: 200, body: { mySegments: [] } });
  ...
  assert.test(t => { // Testing when we start with cached data with split filter but expired, and the same split filter is provided in the config
    ...
    fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=-1&prefixes=p1,p2', { status: 200, body: { splits: [splitDeclarations.p1__split, splitDeclarations.p2__split], since: -1, till: 1457552620999 } }, { delay: 10 }); // short delay to let emit SDK_READY_FROM_CACHE
    fetchMock.getOnce(testUrls.sdk + '/mySegments/nicolas%40split.io', { status: 200, body: { mySegments: [] } });
  ...
  assert.test(assert => { // Testing when we start with cached data with split filter, and no or invalid split filter config (null query string)
    ...
    syncParamsWithNullSplitQuery.forEach(syncParam => {
      ...
      assert.test(t => {
        ...
        fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=-1', { status: 200, body: { splits: [splitDeclarations.p1__split, splitDeclarations.p2__split, splitDeclarations.p3__split], since: -1, till: 1457552620999 } }, { delay: 10 }); // short delay to let emit SDK_READY_FROM_CACHE
        fetchMock.getOnce(testUrls.sdk + '/mySegments/nicolas%40split.io', { status: 200, body: { mySegments: [] } });
  ...
  assert.test(t => { // Testing when we start with cached data with split filter, and a new split filter is provided in the config
    ...
    fetchMock.getOnce(testUrls.sdk + '/splitChanges?v=1.0&since=-1&names=no%20exist%20trim,no_exist,p3__split&prefixes=no%20exist%20trim,p2', { status: 200, body: { splits: [splitDeclarations.p2__split, splitDeclarations.p3__split], since: -1, till: 1457552620999 } }, { delay: 10 }); // short delay to let emit SDK_READY_FROM_CACHE
    fetchMock.getOnce(testUrls.sdk + '/mySegments/nicolas%40split.io', { status: 200, body: { mySegments: [] } });
