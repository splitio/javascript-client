...
fetchMock.get(url(settings, '/splitChanges?v=1.0&since=25'), function () {
  return new Promise((res) => { setTimeout(() => res({ status: 200, body: splitChangesMock1 }), 1000); });
});
fetchMock.get(url(settings, '/splitChanges?v=1.0&since=1500492097547'), { status: 200, body: splitChangesMock2 });
fetchMock.get(url(settings, '/splitChanges?v=1.0&since=1500492297547'), { status: 200, body: splitChangesMock3 });
