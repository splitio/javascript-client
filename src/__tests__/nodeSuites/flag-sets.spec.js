...
export default function flagSets(fetchMock, t) {
  ...
  t.test(async (assert) => {
    ...
    fetchMock.getOnce(baseUrls.sdk + '/splitChanges?s=1.3&since=1602796638344&rbSince=-1&sets=set_1', async function () {
      ...
      // New assertions for impressionsDisabled
      assert.deepEqual(client.getTreatmentsByFlagSet(key, 'set_1', { impressionsDisabled: true }), { workm: 'on' }, 'impressionsDisabled: true supported in getTreatmentsByFlagSet');
      assert.deepEqual(client.getTreatmentsWithConfigByFlagSet(key, 'set_1', { impressionsDisabled: true }), { workm: { treatment: 'on', config: null } }, 'impressionsDisabled: true supported in getTreatmentsWithConfigByFlagSet');
      ...
      assert.deepEqual(client.getTreatmentsWithConfigByFlagSets(key, ['set_1', 'set_2', 'set_3'], { impressionsDisabled: true }), { workm: { treatment: 'on', config: null } }, 'impressionsDisabled: true supported in getTreatmentsWithConfigByFlagSets');
      await client.destroy();
      assert.end();
