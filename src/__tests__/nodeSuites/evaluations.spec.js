...
export default async function (config, key, assert) {
  ...
  const getTreatmentsWithConfigTests = (client, sdkInstance) => {
    ...
    assert.deepEqual(client.getTreatmentWithConfig('a_key', 'split_with_config'), { treatment: 'o.n', config: expectedConfig }, 'If we get the treatment for a Split WITH config, we get such config as a string and the treatment.');
  ...
  for (i; i < SDK_INSTANCES_TO_TEST; i++) {
    ...
    assert.deepEqual(serverSideState.membershipsData, undefined);
    ...
    assert.deepEqual(clientSideState.membershipsData, { user1: { ms: { k: [] }, ls: { k: [] } } });
