...
export function testPushRetriesDueToAuthErrors(fetchMock, assert) {
  ...
  fetchMock.getOnce(url(settings, '/v2/auth?s=1.2'), { throws: new TypeError('Network error') });
  fetchMock.getOnce(url(settings, '/v2/auth?s=1.2'), function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/v2/auth` request must include `Authorization` header');
...
export function testPushRetriesDueToSseErrors(fetchMock, assert) {
  ...
  fetchMock.get({ url: url(settings, '/v2/auth?s=1.2'), repeat: 3 /* 3 push attempts */ }, function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/v2/auth` request must include `Authorization` header');
...
export function testSdkDestroyWhileAuthSuccess(fetchMock, assert) {
  ...
  fetchMock.getOnce(url(settings, '/v2/auth?s=1.2'), { status: 200, body: authPushEnabled }, { delay: 100 });
...
export function testSdkDestroyWhileAuthRetries(fetchMock, assert) {
  ...
  fetchMock.getOnce(url(settings, '/v2/auth?s=1.2'), { status: 200, body: authPushBadToken });
  fetchMock.getOnce(url(settings, '/v2/auth?s=1.2'), { throws: new TypeError('Network error') }, { delay: 100 });
