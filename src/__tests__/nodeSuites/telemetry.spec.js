...
export default async function telemetryNodejsSuite(key, fetchMock, assert) {
  ...
  fetchMock.getOnce(url(config, '/splitChanges?s=1.2&since=-1'), 500); // record http exception
  fetchMock.getOnce(url(config, '/splitChanges?s=1.2&since=-1'), { status: 200, body: splitChangesMock1 });
