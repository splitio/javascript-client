...
export default async function telemetryNodejsSuite(key, fetchMock, assert) {
  ...
  fetchMock.getOnce(url(config, '/splitChanges?v=1.0&since=-1'), 500); // record http exception
  fetchMock.getOnce(url(config, '/splitChanges?v=1.0&since=-1'), { status: 200, body: splitChangesMock1 });
