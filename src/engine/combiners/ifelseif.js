...
function ifElseIfCombinerContext(predicates: Array<Function>): Function {
  ...
  function ifElseIfCombiner(key: SplitKey, seed: number, trafficAllocation: number, trafficAllocationSeed: number, attributes: Object, algo: number): AsyncValue<?Evaluation> {
    ...
    const predicateResults = predicates.map(evaluator => evaluator(key, seed, trafficAllocation, trafficAllocationSeed, attributes, algo));
